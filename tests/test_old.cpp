...
TEST_CASE("Testing VMCLocEnAndPoss_") {
    ...
    const std::string logFileName = "../artifacts/testVMCLocEnAndPoss_-log.txt";
    ...
    SUBCASE("2D harmonic oscillator, one particle") {
        ...
        struct PotHO {
            ...
            vmcp::FPType operator()(vmcp::Positions<2, 1> x) const {
                return std::pow(x[0][0].val + x[0][1].val, 2) * (m.val * omega * omega / 2);
            }
        };
        ...
        SUBCASE("No variational parameters, with Metropolis or importance sampling") {
            struct WavefHO {
                ...
                vmcp::FPType operator()(vmcp::Positions<2, 1> x, vmcp::VarParams<0>) const {
                    return std::exp(-std::pow(x[0][0].val + x[0][1].val, 2) *
            ...
            struct LaplHO {
                ...
                vmcp::FPType operator()(vmcp::Positions<2, 1> x, vmcp::VarParams<0>) const {
                    return ((std::pow(x[0][0].val * m.val * omega / vmcp::hbar, 2) +
                             std::pow(x[1][0].val * m.val * omega / vmcp::hbar, 2) -
                             2 * (m.val * omega / vmcp::hbar))) *
                           WavefHO{m, omega}(x, vmcp::VarParams<0>{});
                }
            };

            WavefHO wavefHO{mInit.val, omegaInit};
            vmcp::Gradients<2, 1, FirstDerHO> gradHO{FirstDerHO{mInit.val, omegaInit}};
            vmcp::Laplacians<1, LaplHO> laplHO{LaplHO{mInit, omegaInit}};
            PotHO potHO{mInit.val, omegaInit};
            ...
            for (auto [i, m_] = std::tuple{vmcp::IntType{0}, mInit}; i != mIterations;
                ...
                for (auto [j, omega_] = std::tuple{vmcp::IntType{0}, omegaInit}; j != omegaIterations;
                    ...
                    vmcp::Energy const expectedEn{vmcp::hbar * omega_ / 2};
                    vmcp::VMCResult const vmcrMetr =
                        vmcp::VMCEnergy<2, 1, 0>(wavefHO, vmcp::ParamBounds<0>{}, laplHO, m_, potHO,
                                                 coordBounds, numberEnergies, rndGen);
                    vmcp::VMCResult const vmcrImpSamp =
                        vmcp::VMCEnergy<2, 1, 0>(wavefHO, vmcp::ParamBounds<0>{}, gradHO, laplHO, m_, potHO,
                                                 coordBounds, numberEnergies, rndGen);
...
TEST_CASE("Testing Statistics") {
    ...
    SUBCASE("Testing BlockingAnalysis_") {
        std::vector<vmcp::Energy> testEnergies = {vmcp::Energy{1.f}, vmcp::Energy{2.f}, vmcp::Energy{3.f},
                                                  vmcp::Energy{4.f}};
        vmcp::BlockingResult blockingResults = BlockingAnalysis_(testEnergies);
    ...
    SUBCASE("Testing BootstrappingAnalysis") {
        ...
        CHECK(std::abs(bootstrapResults.variance.val - 1 / 2) < statisticsTolerance);
    }
}
